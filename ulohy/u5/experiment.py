...
class Experiment:
    ...
    def __initialize_csv_file(self):
        ...
        with open(self.accuracies_file, 'w') as file:
            file.write("Model,Replication,Accuracy,F1 Score,ROC AUC, Sensitivity, Specificity,Best Parameters\n")
    ...
    def __train_and_evaluate_model(self, model_name, X_resampled, y_resampled, replication):
        ...
        accuracy, f1, roc_auc,sensitivity, specificity, predictions = trainer.evaluate(X_test, y_test)

        self.__store_results(model_name, replication, accuracy, f1, roc_auc, sensitivity, specificity, best_params)
        self.replication_conf_matrices[model_name].append(confusion_matrix(y_test, predictions))

    def __store_results(self, model_name, replication, accuracy, f1, roc_auc, sensitivity, specificity, best_params):
        ...
        with open(self.accuracies_file, 'a') as file:
            file.write(f"{model_name},{replication + 1},{accuracy:.4f},{f1:.4f},{roc_auc:.4f}, {sensitivity:.4f}, {specificity:.4f},\"{best_params}\"\n")
