...
def run_experiment(dataset, models, param_grids, logger):
    ...
    experiment = Experiment(models, param_grids, logger=logger, n_replications=30)
...
def plot_results(experiment, results, logger):
    ...
    plotter.plot_evaluation_metric_over_replications(experiment.results.groupby('model')['sensitivity'].apply(list).to_dict(),
        'Sensitivity', 'Sensitivity')

    plotter.plot_evaluation_metric_over_replications(
        experiment.results.groupby('model')['specificity'].apply(list).to_dict(),
